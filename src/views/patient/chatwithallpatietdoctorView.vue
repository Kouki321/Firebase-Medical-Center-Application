  ...
  setup() {
    ...
    const loadChat = async (doctorId) => {
      try {
        ...
        const chatId = `chat_${userId}_${doctorId}`; // Correct chat ID format
    ...
    const sendMessage = async () => {
      if (newMessage.value.trim() && selectedUser.value) {
        try {
         ...
         const chatId = `chat_${userId.value}_${selectedUser.value.id}`; // Correct chat ID format
    ...
    onMounted(async () => {
      try {
        ...
        const listofthisPatientThatAcceptedDoctorID = listofRequestsnapshot.docs
          .map(doc => doc.data())
          .filter(request => request.userId === userId && request.status.includes("accepted"))
        ...
        users.value = usersSnapshot.docs
          ...
          .filter(user => user.doctor === true && user.active === true && listofthisPatientThatAcceptedDoctorID.includes(user.id));
