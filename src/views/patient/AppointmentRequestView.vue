  ...
  <div>
    ...
    <div v-if="showModal" class="modal-overlay">
      <div class="modal-content">
        ...
        <!-- Modal for Appointment Requests -->
    <div v-if="showModalShow" class="modal-overlay">
      <div class="modal-content-detail">
        <button class="close-button" @click="showModalShow = false">X</button>
        <h2>Details Appointment</h2>

        <form>
          <div class="dateA">
            <label for="appointmentDateTimeChange">Date and Time:</label>
            <input
              type="datetime-local"
              v-model="appointmentDateTimeChange"
              :min="minDateTime" readonly
          ...
          <div class="comment">
            ...
            <textarea
              v-model="commentChange" readonly
              placeholder="Add your comments here"
              rows="4"
            ></textarea>
          </div>
          <div class="comment">
            <label for="commentChangeDoctor">Ordonance:</label>
            <textarea
              v-model="commentChangeDoctor" readonly
              placeholder="Add your comments here"
              rows="4"
            ></textarea>
          </div>
                  ...
                  <tr v-for="Appointment in filteredRequestsDoctor" :key="Appointment.id" class="request-card">
                    ...
                    <td class="text-center">
                      <div class="d-flex flex-column justify-content-center">
                        ...
                        :class="{'togelObgoing':Appointment.status==='ongoing','togelac':Appointment.status==='accepted'}">{{ Appointment.status }}</h4>
                      </div>
                    </td>
                    <td class="text-center">
                      <button @click="editAppointment(Appointment.id),showModalEdit=true" :disabled="Appointment.status==='accepted'" class="btn btn-warning">Edit</button>
                    </td> 
                    <td class="text-center">
                      <button @click="detailsAppointment(Appointment.id),showModalShow=true" :disabled="!(Appointment.status==='accepted')" class="btn btn-blue">details</button>
                    </td>
                    <td class="text-center">
...
export default {
  ...
  setup() {
   ...
   const detailsAppointment = async (appointmentId) => {
      appointmentIdChange.value = appointmentId;
      const db = getFirestore();
    ...
    const submitRequest = async () => {
      if (selectedDoctor.value && appointmentDateTime.value) {
        try {
          ...
          //  doctorId: selectedDoctor.value.uid,
    ...
    return {
      showModal,showModalEdit,showModalShow,
      selectedRole,searchQueryDoctor,
      selectedDoctor,
      searchTerm,
      doctors,deleteAppointment,editAppointment,detailsAppointment,
      hisAppointment,
      filteredDoctors,
      appointmentDateTime,
      appointmentDateTimeChange,
          commentChange,commentChangeDoctor,
...
.btn-blue {
background-color: #080bc0;
border: 1px solid #0f18cf;
color: #fff;
}
