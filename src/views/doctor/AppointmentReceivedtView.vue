      ...
      <div v-if="showModal" class="modal-overlay">
        <div class="modal-content">
          ...
          <form @submit.prevent="submitRequest">
            ...
            <img :src="patientDetails.profileImage" class="user-image" />
  
            <div class="selectD">
              <input
                v-model="searchTerm" 
                placeholder="Search patients..." @change="fetchPatient"
                class="search-input"
              />
              <label for="doctor">Select Patient:</label>
              <select v-model="selectedPatient">
                <option v-for="patient in filteredPatient" :key="patient.id" :value="patient.id">
            ...
            <button class="submitbtn" type="submit" :class="{disabled:!selectedPatient || !appointmentDateTime}">
              Submit Request
            </button>
          </form>
        </div>
                    ...
                    <tr v-for="Appointment in filteredRequestsPatient" :key="Appointment.id" class="request-card">
                      ...
                      <td class="text-center">
                        <div class="d-flex flex-column justify-content-center">
                          <h3 class="mb-0 text-sm">{{ Appointment.patient.firstName }}</h3>
                        </div>
                      </td>
                      <td class="text-center">
                        <div class="d-flex flex-column justify-content-center">
                          <h4 class="mb-0 text-sm">{{ Appointment.patient.lastName }}</h4>
                        </div>
                      </td>
                      <td class="text-center">
                        <div class="d-flex flex-column justify-content-center">
                          <h5 class="mb-0 text-sm">{{ Appointment.patient.email }}</h5>
                        </div>
                      ...
                      <td class="text-center">
                        <div class="d-flex flex-column justify-content-center">
                          ...
                          :class="{'togelac':Appointment.status==='accepted',
                          'togelObgoing':Appointment.status==='ongoing',
                          'togeldel':Appointment.status.includes('deleted by Patient')}">{{ Appointment.status }}</h4>
                        </div>
                      </td>
                      <td class="text-center">
                        <button @click="acceptAppointment(Appointment.id)"  class="btn btn-accepted">Accept</button>
...
////
      const fetchPatientsAppointment = async () => {
            ...
            const q = query(hisAppointmentRef, where('status', 'in', ['ongoing', 'accepted', 'deleted by Patient']));
           ...
           const userData = doc.data();
            ...
            onSnapshot(q, (snapshot) => {
              hisAppointment.value = snapshot.docs
                ...
                .filter(appointment => {
                  return appointment.doctor === user.uid;
      ...
      const submitRequestChange = async () => {
        try {
          ...
          await updateDoc(appointmentRef, {
              ...
              commentDoctor: commentChangeDoctor.value,
              commentPrivate: commentChangePrivate.value,
              status: "ongoing"
  ...
  /////////
      const fetchPatient = async () => {
          ...
          const listofthisPatientThatAcceptedDoctorID = listofRequestsnapshot.docs
            .map(doc => doc.data())
            .filter(request => request.doctorId = user.uid && request.status === "accepted")
...
///
      const updateSelectedPatient = (PatientId) => {
      const patient = patients.value.find((doc) => doc.id === PatientId);
      if (patient) {
    ...
    watch(selectedPatient, (newPatientId) => {
      updateSelectedPatient(newPatientId);
    });

      const submitRequest = async () => {
        if (selectedPatient.value && appointmentDateTime.value) {
      ...
      return {
        ...
        selectedPatient,
        searchTerm,
        patients,deleteAppointment,editAppointment,acceptAppointment,
        hisAppointment,
        filteredPatient,
        appointmentDateTime,
