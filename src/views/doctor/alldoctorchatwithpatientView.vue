<template>
    <div class="admin-chat-container">
      ...
      <div class="chat-area" v-if="selectedUser">
        <header class="chat-header">
          ...
          <h1 v-if="selectedUser?.doctor">Chat with Doctor {{ selectedUser?.firstName }} {{ selectedUser?.lastName }}</h1>
          <h1 v-else>Chat with {{ selectedUser?.firstName }} {{ selectedUser?.lastName }}</h1>
        </header>
  ...
  import { getFirestore, collection, query, orderBy, getDocs, onSnapshot, addDoc,doc,getDoc } from 'firebase/firestore';
    ...
    setup() {
      ...
      const sendMessage = async () => {
        if (newMessage.value.trim() && selectedUser.value) {
          try {
           ...
           const chatId = `chat_${selectedUser.value.id}_${userId.value}`; // Correct chat ID format
  
          // Reference to the chat document
     
            const db = getFirestore();
            const messagesRef = collection(db, `chatDoctor_Patient/${chatId}/messages`); // Correct collection path
      ...
      onMounted(async () => {
        try {
          ...
          console.log(listofRequestsnapshot.length,"listofRequestsnapshot");

          const listofthisPatientThatAcceptedDoctorID = listofRequestsnapshot.docs
            .map(doc => doc.data())
            .filter(request => request.doctorId === userId && request.status.includes("accepted"))
            .map(request => request.userId);
            console.log(listofthisPatientThatAcceptedDoctorID.length,"listofthisPatientThatAcceptedDoctorID");
          ...
          users.value = usersSnapshot.docs
            ...
            .filter(user => user.patient === true && user.active === true && listofthisPatientThatAcceptedDoctorID.includes(user.id));
